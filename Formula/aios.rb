      ...
      if system "which python3.10 >/dev/null 2>&1"
        ...
        (bin/"hexel").write <<~EOS
          ...
          if [ "$1" = "run" ]; then
            ...
            cd #{libexec}
            nohup #{system_python} -m uvicorn server1:app --host 0.0.0.0 --port 8000 > #{var}/hexel.log 2>&1 &
            echo $! > "$pid_file"
        ...
        (bin/"hexel").write <<~EOS
          ...
          if [ "$1" = "run" ]; then
            ...
            cd #{libexec}
            nohup #{libexec}/bin/uvicorn server1:app --host 0.0.0.0 --port 8000 > #{var}/hexel.log 2>&1 &
            echo $! > "$pid_file"
            echo "AIOS started on port 8000"
