...
rm -rf "$INSTALL_DIR/src/hexel-figs"
    ...
    "env")
        case "$2" in
            ...
            "remove")
                ...
                ;;
            *)
                echo "Usage: hexel env {add|list|remove}"
                ;;
        esac
        ;;
    "clean")
        ...
        ;;
    *)
        echo "Usage: hexel {start|stop|restart|update|env|clean}"
        echo "For environment variables: hexel env {add|list|remove}"
        exit 1
        ;;
esac
EOF
...
# Make it executable
chmod +x /usr/local/bin/hexel

echo "AIOS installed successfully!"
echo "Use:"
echo "  - 'hexel start' to start the server"
echo "  - 'hexel stop' to stop the server"
echo "  - 'hexel restart' to restart the server"
echo "  - 'hexel update' to update to the latest version"
echo "  - 'hexel env add' to add new environment variables"
echo "  - 'hexel env list' to view current environment variables"
echo "  - 'hexel env remove' to remove environment variables"
echo "  - 'hexel clean' to remove AIOS installation"