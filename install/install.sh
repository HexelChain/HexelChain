...
case "$1" in
    ...
    "refresh")
        if [ -f "$PID_FILE" ]; then
            ...
            HOST=$(python -c "from hexel.config.config_manager import config; print(config.config.get('server', {}).get('host', 'localhost'))")
            PORT=$(python -c "from hexel.config.config_manager import config; print(config.config.get('server', {}).get('port', 8000))")
            curl -X POST "http://${HOST}:${PORT}/core/refresh"
        else
            echo "Server is not running. Please start the server first."
            echo "Run: hexel start"
        fi
        ;;
    "env")
        case "$2" in
...
Commands:
  ...
  clean         Uninstall AIOS completely
                ...
                - Removes the hexel command
                Warning: This action cannot be undone!

Examples:
  hexel start             # Start the server
  hexel env add          # Add a new API key or configuration value
  hexel update           # Update to the latest version

Notes:
  - Server runs at http://localhost:8000 by default
  - Log file location: ~/.hexel-1/server.log
  - Configuration directory: ~/.hexel-1
  - Environment file: ~/.hexel-1/.env
...
# Make it executable
chmod +x "$BIN_DIR/hexel"

cat << 'COMPLETE'
🎉 AIOS installed successfully!

The 'hexel' command is now available in your terminal.
Installation directory: ~/.hexel-1
